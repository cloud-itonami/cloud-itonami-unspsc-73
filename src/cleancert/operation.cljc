(ns cleancert.operation
  "CleaningOperationActor -- one cleaning/certification coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (CleaningAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Industrial
  Cleaning Governor (:govern) and the rollout phase gate (:decide)
  ...
  lifecycle is advanced by MANY ops (cleaning-job logging /
  certification-scan decisions / re-clean scheduling), each
  independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human contractor-operator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [cleancert.advisor :as advisor]
...
(defn build
  "Compiles a CleaningOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `cleancert.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CleaningAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Industrial Cleaning Governor -- independent censor.
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :requires-operator-approval
